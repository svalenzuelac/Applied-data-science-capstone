...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                def pie(site):
                                    if site == 'ALL':
                                        ddf = pd.DataFrame(spacex_df.groupby(['Launch Site']))
                                        fig = px.pie(ddf, values=['Class'], names=['Launch Site'], title='Success Counts for Launch Sites in US')

                                    else:
                                        filtered_df= spacex_df[spacex_df['Launch Site'] == site]
                                        filtered_ddf = pd.DataFrame(filtered_df.groupby(['Launch Site']))
                                        fig = px.pie(filtered_ddf, values=['Class Count'], names=['Class'], title='Succes count of '+site)
